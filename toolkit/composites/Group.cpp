...
namespace terminal {
  namespace toolkit {
    struct Group::GroupImpl {
      GroupImpl(Composite *parent) :
        window_(0)
      {
        if(parent) {
    ...
    void Group::setText(const std::wstring &text) {
      pimpl_->text_ = text;
    }

    const std::wstring &Group::getText() const {
      return(pimpl_->text_);
    }
