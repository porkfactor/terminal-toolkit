...
namespace terminal {
  namespace toolkit {
    ...
    void Combo::add(const std::wstring &s) {
      pimpl_->items_.push_back(s);
    }

    void Combo::add(const std::wstring &s, Control::index_t index) {
    ...
    std::wstring Combo::getItem(Control::index_t index) const {
    ...
    uint32_t Combo::indexOf(const std::wstring &s) const {
      return(indexOf(s, 0));
    }

    uint32_t Combo::indexOf(const std::wstring &s, Control::index_t index) const {
      uint32_t rv = -1;
    ...
    void Combo::remove(const std::wstring &s) {
